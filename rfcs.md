...
如果开发者提交了本应该走 RFC 流程的 Pull Request，BaseMetas Fileview 团队会把 Pull Request 关闭并请发起 Pull Request 的开发者在 RFC 仓库执行 RFC 流程。
...
简而言之，如果需要 RFC 改动在未来的 Fileview 版本在发布，需要先让 RFC 提案文档在 RFC 仓库合并。

- Fork BaseMetas Fileview RFC 仓库： https://github.com/basemetas/fileview-rfcs ；
...
- 向 BaseMetas Fileview RFC 仓库发起 Pull Request。在这个阶段 Fileview 团队和社区一同讨论 RFC 提案；
- RFC 提案在社区中达成共识，RFC 提案进入 `Active` 阶段；
- RFC 提案的代码实现在 Fileview 主仓库被合并，RFC 提案进入 `Landed` 阶段，RFC 提案也会被合并进入 RFC 仓库；
